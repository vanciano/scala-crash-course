...
class Empty extends TweetSet:
  ...
  override def mostRetweeted: Tweet = throw NoSuchElementException()
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def mostRetweeted: Tweet =
    var cur = elem
    this.foreach(t => if (t.retweets > cur.retweets) then cur = t )
    cur

  override def union(that: TweetSet): TweetSet =
    var upd: TweetSet = that
    foreach { t => upd = upd.incl(t) }
    upd

  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    val newAcc = p(elem) match {
      case true => acc.incl(elem)
      case false => acc
    }
...
object GoogleVsApple:
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(k => t.text.contains(k)))
  val appleTweets: TweetSet = TweetReader.allTweets.filter(t => apple.exists(k => t.text.contains(k)))
