...
object Anagrams extends AnagramsInterface:
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    if occurrences.isEmpty then List(Nil)
      ...
      for
        combos <- if occurrences.tail.isEmpty then  List(Nil) else combinations(occurrences.tail)
        (char, times) = occurrences.head
        i <- 0 to times
      yield Option.when(i != 0) { List((char, i)) }.getOrElse(Nil) ++ combos
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val sub = y.toMap
    x.map((char, int) =>
      if sub.contains(char) then (char, int - sub(char))
      else (char, int)
    ).filter((_, int) => int > 0)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    val sentenceWord = sentenceOccurrences(sentence)
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] =
      if occurrences.isEmpty then List(Nil)
        ...
        for
          ...
          remainingCombinations <- anagrams(subtract(occurrences, combination))
        yield word :: remainingCombinations

    anagrams(sentenceWord)
